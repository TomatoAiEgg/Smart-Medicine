...
本文档记录新系统模块边界。模块划分以 `docs/03_架构设计/重构方案.md` 为准，先定义核心模块，再定义扩展模块。
...
| `ops-service` | Spring Boot 服务 | 操作日志、异常日志、审计、链路追踪、监控聚合、联调记录；当前已覆盖基础日志查询、物流/回调失败原因聚合、集成重试失败聚合和核心健康概览 | 后台日志、运维散落能力 |
...
| 模块 | 技术 | 核心页面 |
| --- | --- | --- |
| `admin-web` | Vue3 + TypeScript + Vite | 订单中心、处方中心、药房作业、煎煮作业、物流发货、门户集成、报表统计、运维审计、平台配置；专项重构以 `docs/03_架构设计/admin-web前端重构方案.md` 为准，当前已包含订单查询与履约进度、审核/调剂/复核任务、煎煮模拟、物流回调、门户查单、报表统计与导出、集成适配重试派发、回调失败/集成失败排错和运维健康概览入口 |
