...
public class OrderRepository {
    ...
    public Optional<AdminOrderDetail> findAdminOrderDetailByOrderNo(String orderNo) {
        String sql = """
                select
                    ...
                    o.legacy_company_num,
                    o.create_ip,
                    o.order_time,
                    o.classes,
                    o.order_pkg_weight,
                    o.order_pkg_num,
                    o.logistics_receivables_money,
                    o.logistics_pay_method,
                    o.logistics_type,
                    o.logistics_mode,
                    o.sp_order_id,
                    o.logis_id,
        ...
        return jdbcTemplate.query(sql, this::mapAdminOrderDetailHeader, orderNo)
                ...
                .map(header -> new AdminOrderDetail(
                        ...
                        header.legacyCompanyNum(),
                        header.createIp(),
                        header.orderTime(),
                        header.classes(),
                        header.orderPkgWeight(),
                        header.orderPkgNum(),
                        header.logisticsReceivablesMoney(),
                        header.logisticsPayMethod(),
                        header.logisticsType(),
                        header.logisticsMode(),
                        header.spOrderId(),
                        header.logisId(),
                        header.areaLevel(),
                        header.routeCode(),
                        header.baseProductNo(),
                        header.packageTime(),
                        header.outboundTime(),
                        header.signTime(),
    ...
    private List<AdminOrderDetail.Prescription> findAdminOrderDetailPrescriptions(UUID orderId) {
        String sql = """
                select
                    ...
                    p.hospital_name,
                    p.hospital_num,
                    p.order_handle_floor,
                    p.jyj_decoction_plan,
                    p.jyj_decoction_advice,
                    p.label_size,
                    p.bind_no,
                    p.drugs_money,
                    p.audit_flow_pic_url,
                    p.audit_reason,
                    p.audit_result,
                    p.dispense_flow_pic_url,
                    p.recheck_flow_pic_url,
                ...
                group by p.id, p.prescription_no, p.external_prescription_no, p.prescription_type,
                         ...
                         p.medication_method, p.medication_instruction, p.prescription_remark,
                         p.patient_age, p.patient_month_age, p.patient_day_age, p.patient_gender,
                         p.patient_card_no, p.treat_card, p.patient_tel, p.is_pregnant,
                         p.herb_type, p.wj_type, p.doctor_tel, p.hospital_name, p.hospital_num,
                         p.order_handle_floor, p.jyj_decoction_plan, p.jyj_decoction_advice,
                         p.label_size, p.bind_no, p.drugs_money, p.audit_flow_pic_url,
                         p.audit_reason, p.audit_result, p.dispense_flow_pic_url,
                         p.recheck_flow_pic_url, p.created_at
        ...
        return jdbcTemplate.query(sql, this::mapAdminOrderDetailPrescription, orderId)
                ...
                .map(prescription -> new AdminOrderDetail.Prescription(
                        ...
                        prescription.patientMonthAge(),
                        prescription.patientDayAge(),
                        prescription.patientGender(),
                        prescription.patientCardNo(),
                        prescription.treatCard(),
                        prescription.patientTel(),
                        prescription.isPregnant(),
                        prescription.herbType(),
                        prescription.wjType(),
                        prescription.doctorTel(),
                        prescription.hospitalName(),
                        prescription.hospitalNum(),
                        prescription.orderHandleFloor(),
                        prescription.jyjDecoctionPlan(),
                        prescription.jyjDecoctionAdvice(),
                        prescription.labelSize(),
                        prescription.bindNo(),
                        prescription.drugsMoney(),
                        prescription.auditFlowPicUrl(),
                        prescription.auditReason(),
                        prescription.auditResult(),
                        prescription.dispenseFlowPicUrl(),
                        prescription.recheckFlowPicUrl(),
    ...
    private QueryParts adminOrderFilters(AdminOrderSearchQuery query) {
        ...
        filters.addLikeFilter("coalesce(o.storage_type, i.storage_type)", query.decoctionCenter());
    ...
    private AdminOrderDetailHeader mapAdminOrderDetailHeader(ResultSet rs, int rowNum) throws SQLException {
        return new AdminOrderDetailHeader(
                ...
                rs.getString("legacy_company_num"),
                rs.getString("create_ip"),
                instant(rs, "order_time"),
                rs.getString("classes"),
                rs.getBigDecimal("order_pkg_weight"),
                integer(rs, "order_pkg_num"),
                rs.getBigDecimal("logistics_receivables_money"),
                rs.getString("logistics_pay_method"),
                rs.getString("logistics_type"),
                rs.getString("logistics_mode"),
                rs.getString("sp_order_id"),
                rs.getString("logis_id"),
    ...
    private AdminOrderDetail.Prescription mapAdminOrderDetailPrescription(ResultSet rs, int rowNum)
        ...
        return new AdminOrderDetail.Prescription(
                ...
                rs.getString("patient_age"),
                rs.getString("patient_month_age"),
                rs.getString("patient_day_age"),
                rs.getString("patient_gender"),
                rs.getString("patient_card_no"),
                rs.getString("treat_card"),
                rs.getString("patient_tel"),
                rs.getString("is_pregnant"),
                rs.getString("herb_type"),
                rs.getString("wj_type"),
                rs.getString("doctor_tel"),
                rs.getString("hospital_name"),
                rs.getString("hospital_num"),
                rs.getString("order_handle_floor"),
                rs.getString("jyj_decoction_plan"),
                rs.getString("jyj_decoction_advice"),
                rs.getString("label_size"),
                rs.getString("bind_no"),
                rs.getBigDecimal("drugs_money"),
                rs.getString("audit_flow_pic_url"),
                rs.getString("audit_reason"),
                rs.getString("audit_result"),
                rs.getString("dispense_flow_pic_url"),
                rs.getString("recheck_flow_pic_url"),
                rs.getInt("detail_count"),
                instant(rs, "created_at"),
                List.of()
        );
    ...
    private AdminOrderDetail.DrugDetail mapAdminOrderDetailDrugDetail(ResultSet rs, int rowNum) throws SQLException {
        return new AdminOrderDetail.DrugDetail(
                ...
                rs.getString("dc_goods_num"),
                rs.getString("dc_goods_name"),
                rs.getString("roots_goods_name"),
                rs.getString("supplier_name"),
                rs.getBigDecimal("med_per_dose"),
                rs.getBigDecimal("med_per_day"),
                rs.getString("detail_status"),
                rs.getString("note"),
                rs.getBigDecimal("water_absorb_ratio"),
                instant(rs, "created_at")
        );
    }
